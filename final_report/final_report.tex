...
Next, the profiler registers forward and backward hooks. Since the PyTorch Autograd engine accumulates values necessary to walk back the computational graph (i.e. backpropagation) internally in C++ buffers, hooks provide otherwise-unavailable access to intermediate tensors. Hooks allow the profiler to inspect feature maps and gradients, respectively, as PyTorch creates them. The hooks that are used are specialized functions defined within the profiler to gather memory consumption metrics.
...
\end{myitemize}
The usage of the profiler will be demonstrated using the Neural Collaborative Filtering \cite{ncf} model as an example. Listing \ref{listing:ncf_model_init} shows an example from training code \cite{tbd_suite} which instantiates a model. On line 14, the model is sent to the GPU.
