...
During the training algorithm, the forward and backward pass execute as normal. However, the hooks registered earlier are automatically run after every intermediate computation. The profiler keeps track of all the memory usage by each part of the model, and reports relevant diagnostics. The cache is also monitored, and statistics about its peak size and its current size at the end of each iteration are reported. \par 

%%%%%%%%% RESULTS
\section{PyTorch Memory Profiler Use and Results}
\label{results}
This section presents an example of how the profiler is used, along with profiling results on a GTX 1080 Ti GPU.
% TODO: my cpu, OS, ram, etc.
