...
Tensors can only get a user so far. Ultimately, machine learning researchers need an interface to codify their machine learning models, which can then be trained and used. The main abstraction used to do this in PyTorch is the \texttt{nn.Module} class. It is something that PyTorch users quickly become accustomed to using, since it is employed to implement 
\par 
\begin{lstlisting}[language=Python, caption={}, label={listing:nn.module_example}]
