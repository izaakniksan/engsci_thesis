...
\section{Future Work}
\label{future_work}
A user-friendly and feature-complete profiler is still required. This requires combining the profiler components together, and further investigation about how to cleanly integrate the profiler with a user's PyTorch code. The current approach has been to insert a few API calls at specific sections of the training run - for example right before and after the \texttt{.backward()} call - which informs the user about the total memory usage of the program (including the cache), the total amount of the cache currently used by tensors, and a layer-by-layer breakdown of these tensors. The metrics gathered by the profiler must then be exported into graphs, which can be done using Matplotlib \cite{matplotlib}.
\par
